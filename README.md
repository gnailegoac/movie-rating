...
系统不会直接把三个平台的原始分相加。它先用三家都有评分的同一批影片计算各平台均值 `μp` 和标准差 `σp`，再把原始分映射到共同尺度：

```text
平台校准分 = 7.5 + (平台原始分 - μp) / σp × 1.0
```

结果限制在 0–10 分。这样，同样处在各自平台“高于均值一个标准差”的电影都会得到 8.5 的校准分，不会把猫眼或淘票票较集中的 9.x 直接等同于豆瓣 9.x。至少需要 5 部三平台评分完整的共同影片才启用校准；样本不足时回退到原始分。
...
缺少某个平台评分时，不把缺失值当作 0 分，而是把剩余平台权重按原比例归一化。例如淘票票缺失时，豆瓣与猫眼的 50:25 会变成 67%:33%。页面同时显示平台原始分和参与公式的校准分。用户可以临时调整权重，榜单和详情会即时重算；这只改变当前浏览器里的展示，不改写数据缓存。
