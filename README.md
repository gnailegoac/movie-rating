...
结果限制在 0–10 分，Z 值同时限制在 ±2.5，避免小样本或极端评分被狭窄的标准差过度放大。这样，同样处在各自平台“高于均值一个标准差”的电影都会得到 8.5 的校准分，不会把猫眼或淘票票较集中的 9.x 直接等同于豆瓣 9.x。至少需要 5 部三平台评分完整的共同影片才启用校准；样本不足时回退到原始分。
...
每次同步会读取淘票票“正在热映”列表，挑选已经开分的影片，通过猫眼精确匹配补充中国上映日期、导演、类型和片长，再尝试匹配豆瓣条目。满足至少两个有效评分来源的新片会自动加入查询库。已经收录的影片退出在映列表后改为 `archived`，仍保留在历史查询中，不会从缓存删除。

默认每轮最多检查当前列表前 24 部已开分影片，可通过 `DISCOVERY_LIMIT` 调整。豆瓣发生限流时，新片会先以猫眼和淘票票评分入库，后续任务优先补齐缺失来源；分布校准仍只使用三家评分都完整的共同影片。
...
1. 编辑 `data/ratings-source.json`，维护核心种子影片和三平台影片 ID；其中的评分也是首次同步失败时的安全基线，完整片库会自动增长。
