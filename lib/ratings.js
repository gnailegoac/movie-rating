export const PLATFORMS = ["douban", "mtime", "maoyan", "taopiaopiao"];
export const ANCHOR_PLATFORMS = ["douban", "mtime"];
...
/** Estimated scores contribute less evidence than an official platform score. */
export function ratingConfidence(rating) {
  if (!Number.isFinite(rating?.score)) return 0;
  if (!Number.isFinite(rating?.confidence)) return 1;
  return Math.min(1, Math.max(0, Number(rating.confidence)));
}

/** A composite needs at least one community/editorial source, not ticketing scores alone. */
export function hasRatingAnchor(ratings, weights = DEFAULT_WEIGHTS) {
  const normalized = normalizeWeights(weights);
  return ANCHOR_PLATFORMS.some((platform) =>
    normalized[platform] > 0 && ratingConfidence(ratings?.[platform]) > 0,
  );
}
...
export function calculateCalibration(movies) {
  const completeCases = (movies ?? []).filter((movie) =>
    PLATFORMS.every((platform) =>
      Number.isFinite(movie.ratings?.[platform]?.score)
      && movie.ratings?.[platform]?.scoreType !== "estimated-subitems",
    ),
  );
...
/** @param {{douban?: number, mtime?: number, maoyan?: number, taopiaopiao?: number}} weights */
...
export function effectiveWeights(ratings, weights = DEFAULT_WEIGHTS) {
  if (!hasRatingAnchor(ratings, weights)) {
  ...
  const confidenceAdjusted = Object.fromEntries(
    PLATFORMS.map((platform) => [platform, normalized[platform] * ratingConfidence(ratings?.[platform])]),
  );
  const availableWeight = available.reduce((sum, platform) => sum + confidenceAdjusted[platform], 0);
  ...
  return Object.fromEntries(
    PLATFORMS.map((platform) => [
      platform,
      available.includes(platform) ? confidenceAdjusted[platform] / availableWeight : 0,
    ]),
  );
}
...
export function calculateCoverage(ratings, weights = DEFAULT_WEIGHTS) {
  if (!hasRatingAnchor(ratings, weights)) return 0;
  const normalized = normalizeWeights(weights);
  return Math.round(
    PLATFORMS.reduce(
      (sum, platform) => sum + normalized[platform] * ratingConfidence(ratings?.[platform]),
      0,
    ) * 100,
  );
}
